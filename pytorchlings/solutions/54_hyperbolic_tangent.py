"""Solution 54: simple hyperbolic-inspired message passing.
...
class HyperbolicLikeLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        # map from Poincare ball to tangent: logmap0(x) ~= atanh(||x||) * x/||x||.
        eps = 1e-6
        norm = x.norm(dim=-1, keepdim=True).clamp(min=eps, max=1 - eps)
        tangent = torch.atanh(norm) * x / norm
        # aggregate in tangent space and project back with tanh.
        agg = adj @ tangent
        out = self.lin(agg)
