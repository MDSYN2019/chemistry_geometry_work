...
2. Calculate the loss - the mode's outputs are compared to the ground truth and evaluated to see how wrong they are

3. Zero gradients - The optimizers gradients are set to zero so they can be recauclated for the
   specific training step

4. Perfrom backpropagation on the loss - Computes the gradient of the loss with respect for every model
...
def training_loop(epochs: int, model: torch.nn.Module, loss_fn: torch.nn.Module, optimizer: torch.optim.Optimizer,
                  ...
                  X_test: torch.Tensor, y_test: torch.Tensor, writer) -> None:
    """
    ...
    --
    This needs to be generalized between classification and regression models
    """
    for epoch in tqdm(range(epochs)):
        logging.info(f"We are on epoch {epoch}")
...
class FashionMNISTDataModule:
    def __init__(self, batch_size = 64, resize = (28, 28), root = './data'):
        self.batch_size = batch_size # Size of each batch for the neural network to digest 
        self.resize = resize 
        self.root = root
...
class BasicClassification(nn.Module):
    def __init__(self):
        ...
        But since we are changing the shape of the input image, this is not a convolutional NN 
        """
        super().__init__()
